...
from riskprobe.monitoring.injection import (
    DetectionScore,
    DriftScenario,
    evaluate_alerts,
    inject_drift,
...
def _service(config: Path, runs_dir: Path) -> RiskProbeService:
    try:
...
def _monitoring_inputs(
...
) -> tuple[pl.DataFrame, object, FeatureCatalog]:
    ...
    frame = dataset.collect(
        [config.columns.segment, config.columns.target, *features]
    )
    return (
        frame,
        profile,
        FeatureCatalog.from_columns(features, config.features.families),
    )


def _safe_alert_payload(
    alert: object, *, expose_segment_values: bool
) -> dict[str, object]:
...
def snapshot(
    ...
    try:
        ...
        _fail(
            "snapshot_error",
            "Check the local configuration and source data, then rerun snapshot.",
        )
    typer.echo(
        json.dumps(
            {
...
def monitor(
    reference_run_id: str = typer.Option(
        ..., help="Existing monitoring reference run ID."
    ),
    current_config: Path = typer.Option(
        ..., help="Local YAML configuration for current data."
    ),
    ...
    try:
        reference = ReferenceSnapshot.model_validate_json(
            (
                runs_dir
                / "monitoring"
                / reference_run_id
        ...
        _write_monitoring_json(
            ...
            [
                _safe_alert_payload(
                    item,
                    expose_segment_values=current.privacy.expose_segment_values,
                )
                for item in alerts
            ],
        )
        ...
        _fail(
            "monitor_error",
            "Check the aggregate reference run and current local configuration.",
        )
    typer.echo(
        json.dumps({"alert_count": len(alerts), "command": "monitor"}, sort_keys=True)
    )
...
def evaluate_drift(
    ...
    """Inject six reproducible drift scenarios and report aggregate scores."""
    try:
        ...
        baseline_card = EvidenceCard(
            rule=RiskRule(
                rule_id="synthetic_monitor_rule",
                conditions=(),
                origin="evaluation",
            ),
            train=metrics,
        ...
        for offset, scenario in enumerate(scenarios):
            ...
            if scenario.drift_type == "population_shift":
                truth = truth.model_copy(
                    update={"expected_scope_value": institution}
                )
            elif scenario.drift_type == "label_shift":
                truth = truth.model_copy(
                    update={"expected_scope_value": reference.dataset_id}
                )
            elif scenario.drift_type == "rule_decay":
                current_cards = (
                    baseline_card.model_copy(
                        update={
                            "test": metrics.model_copy(update={"lift": 1.0})
                        }
                    ),
                )
                truth = truth.model_copy(
                    update={"expected_scope_value": baseline_card.rule.rule_id}
                )
            alerts = detect_anomalies(
                reference, injected.frame, current_cards, catalog
            )
            diagnoses = diagnose_alerts(
                alerts, reference, injected.frame, catalog, top_k=3
            )
        ...
        score = DetectionScore(
            precision=sum(
                item.precision for item in scenario_scores.values()
            )
            / len(scenario_scores),
            recall=sum(item.recall for item in scenario_scores.values())
            / len(scenario_scores),
            false_positive_rate=None,
            false_discovery_rate=sum(
                item.false_discovery_rate for item in scenario_scores.values()
            )
            / len(scenario_scores),
            top_k_root_cause_hit=sum(
                item.top_k_root_cause_hit for item in scenario_scores.values()
            )
            / len(scenario_scores),
        )
            ...
            [
                _safe_alert_payload(
                    ...
                    expose_segment_values=(
                        project_config.privacy.expose_segment_values
                    ),
                )
        ...
        _fail(
            "evaluation_error",
            "Check the local configuration and configured numeric features.",
        )
    typer.echo(
        json.dumps(
            {"command": "evaluate-drift", **score.model_dump(mode="json")},
            sort_keys=True,
        )
    )
...
def prepare_home_credit_command(
    input_dir: Path = typer.Option(
        ..., help="Directory containing user-downloaded Home Credit CSVs."
    ),
    output: Path = typer.Option(..., help="Local output Parquet path."),
) -> None:
    """Prepare public Home Credit behavior features without prediction files."""
    try:
        ...
        _fail(
            "home_credit_preparation_error",
            "Check public CSV inputs and writable output path.",
        )
    typer.echo(
        json.dumps(
            {
...
def benchmark(
    ...
    baseline_record: Path = typer.Option(
        ..., help="Human-authored local baseline JSON record."
    ),
) -> None:
    """Measure local workflow stages without changing the manual baseline."""
    try:
        project_config = ProjectConfig.from_yaml(config)
        runs_dir = _protected_path(runs_dir)
        ...
        if (
            ...
            raise ValueError(
                "baseline record identity does not match current dataset and configuration"
            )
        timings: list[StageTiming] = []
        started = time.perf_counter()
        service.inspect()
        timings.append(
            StageTiming(stage="inspect", seconds=time.perf_counter() - started)
        )
        started = time.perf_counter()
        service.discover()
        timings.append(
            StageTiming(stage="discover", seconds=time.perf_counter() - started)
        )
        started = time.perf_counter()
        context = service.run()
        timings.append(
            StageTiming(stage="validate", seconds=time.perf_counter() - started)
        )
        started = time.perf_counter()
        service.monitoring_snapshot()
        timings.append(
            StageTiming(stage="monitor", seconds=time.perf_counter() - started)
        )
        started = time.perf_counter()
        (context.run_dir / "risk_report.md").read_bytes()
        timings.append(
            StageTiming(stage="report", seconds=time.perf_counter() - started)
        )
...
def resume_evidence(
    records_dir: Path = typer.Option(
        ..., help="Ignored local directory containing benchmark records."
    ),
    output: Path = typer.Option(..., help="Ignored internal Markdown output path."),
) -> None:
    """Create internal resume text from complete measured benchmark records."""
    try:
...
def register_cross_client_commands(app: typer.Typer) -> None:
    """Register cross-client and monitoring commands on the primary CLI."""
    app.command("snapshot")(snapshot)
    app.command("monitor")(monitor)
    app.command("evaluate-drift")(evaluate_drift)
    app.command("prepare-home-credit")(prepare_home_credit_command)
    app.command("preflight-company")(preflight_company)
    app.command("benchmark")(benchmark)
    app.command("resume-evidence")(resume_evidence)
